...
import os
import numpy as np
from src.models.swift_dock_logger import swift_dock_logger
            ...
            for key, value in descriptors_dictionary.items():
                ...
                for size in training_sizes:
                    ...
                    predictions = pd.read_csv(
                        f"{test_predictions_dir}{regressor}_{target}_{key}_{str(size)}_test_predictions.csv")
                ...
                df = pd.DataFrame(
                    list(zip(target_list, train_r2_list, test_mae_list, test_mse_list, test_r2_list, correlations)),
...
def get_a_specific_result(target, training_size, model, metric_type, descriptor):
    ...
    if file_exists is False:
        return 0
    if metric_type in ['test_mse', 'test_mae'] and model in ['decision_tree', 'sgdreg', 'xgboost'] and target in [
...
def correlation_graph(target, descriptor, model_lists, label_list, training_sizes):

    logger.info("Correlation Plot Generation Has Started")
    ...
    for model in model_lists:
        ...
        for train_size in training_sizes:
            ...
            data = pd.read_csv(file_name)
            target_data = data['target'].round(2).tolist()
            predictions_data = data['f0'].round(2).tolist()
            correlation = stats.spearmanr(target_data, predictions_data)[0]
            each_train_size_list.append(round(correlation, 2))

        correlation_list.append(each_train_size_list)

    # Use a predefined style for better visuals
    ...
    base_colors = ['orange', 'blue', 'green', 'salmon']

    max_val = max([max(correlation) for correlation in correlation_list])

    # Create bars dynamically with gradient colors
    # ...

    # Correctly map the model colors to their corresponding colormaps
    color_map_dict = {
        'orange': 'Oranges',
        'blue': 'Blues',
        'green': 'Greens',
    ...
    for i, (correlation, model_name, base_color) in enumerate(zip(correlation_list, model_names, base_colors)):
        color_map = color_map_dict[base_color]
        gradient_colors = [plt.get_cmap(color_map)(x) for x in np.linspace(0.5, 1, len(correlation))]
        bar_positions = [j + width * i for j in x]
        bars = ax.bar(bar_positions, correlation, width, color=gradient_colors, label=model_name, edgecolor="black")
        ax.bar_label(bars, padding=3)

    # ...

    # Labels, title and settings
    ax.set_ylabel('Correlation score', fontsize=14, weight='bold')
    ax.set_xlabel('Training set size', fontsize=14, weight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(label_list)
    ax.set_ylim(0, max_val + 0.1)
    ax.legend(fancybox=True, framealpha=1, loc='lower left', shadow=True, borderpad=1, fontsize=10)

    fig.tight_layout()
    plt.show()
    fig.savefig(f"{graph_results_dir}{target}_correlation_plot", facecolor='w')
    logger.info("Correlation Plot Generation Has Ended")
...
def ven_diagram_for_single_target(targets, model, training_size, descriptor, tok_k):
    ...
    for idx, target in enumerate(targets):
        ...
        v = venn2(subsets=(tar_len, pred_len, union_len), set_labels=['Target', 'Prediction'],
                  set_colors=('blue', 'red'), alpha=0.5, ax=axes[current_row, current_col])
        for text in v.set_labels:
            text.set_fontsize(20)
...
def plot_tanimoto_distances_version_two(target_one, target_two):
    ...
    tanimoto_stats_two = pd.read_csv(f"{tanimoto_results_dir}/{target_two}/all_distances.csv").sample(
        len(tanimoto_stats_one))
